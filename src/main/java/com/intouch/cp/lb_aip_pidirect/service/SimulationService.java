...
public class SimulationService {
    ...
    // Simulation state for each server
    ...
    public void generateSimulatedMetrics() {
        ...
        try {
            ...
            // Update simulation scenario every 5 cycles (5 minutes)
    ...
    private ServerMetrics generateMetricsForServer(String serverId) {
        ...
        switch (serverId) {
            case "server1":
                if (server1Degraded) {
                    baseResponseTime = 800.0; // Degraded performance
                    baseDegradation = 400.0;
                    baseUptime = 95.0;
            ...
            case "server2":
                if (server2HighErrors) {
                    baseErrorRate = 8.0; // High error rate
                    baseTimeoutRate = 3.0;
                    baseUptime = 92.0;
            ...
            case "server3":
                // Occasionally simulate brief outages
        ...
        metrics.setErrorRatePercentage(Math.max(0.0, addVariation(baseErrorRate, variation)));
        metrics.setTimeoutRatePercentage(Math.max(0.0, addVariation(baseTimeoutRate, variation)));
        metrics.setUptimePercentage(Math.min(100.0, Math.max(0.0, addVariation(baseUptime, variation))));
        metrics.setDegradationScore(Math.max(0.0, addVariation(baseDegradation, variation)));
        metrics.setRequestPerMinute((int) Math.max(1, addVariation(baseRequestsPerMinute, variation * 0.5)));
    ...
    // Manual simulation controls for testing
    ...
    // Generate a specific metrics sample for testing
    public ServerMetrics generateSpecificMetrics(String serverId,
        ...
        metrics.setDegradationScore(responseTime / 4.0); // Simple degradation calculation
        metrics.setRequestPerMinute(ThreadLocalRandom.current().nextInt(50, 150));
